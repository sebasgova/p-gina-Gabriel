...
export async function saveProjectAction(id: string | null, formData: FormData) {
  ...
  const [thumbnailUrl, videoUrl, beforeImageUrl, afterImageUrl, beforeVideoUrl, afterVideoUrl] = await Promise.all([
    ...
    resolveMedia(formData, "beforeVideoFile", "beforeVideoUrl", "removeBeforeVideo", existing?.beforeVideoUrl),
    resolveMedia(formData, "afterVideoFile", "afterVideoUrl", "removeAfterVideo", existing?.afterVideoUrl),
  ]);
  ...
  const base: Omit<Project, "id" | "order" | "slug"> = {
    ...
    thumbnailPalette: [str(formData, "paletteFrom") || "#ff6a00", str(formData, "paletteTo") || "#0b0b0d"],
    thumbnailUrl,
    videoUrl,
    beforeImageUrl,
    afterImageUrl,
  ...
  if (id) await updateProject(id, base);
  else await createProject(base);
  revalidateAll();
}
...
export async function deleteProjectAction(id: string) {
  ...
  await Promise.all([
    ...
    deleteUpload(project?.beforeVideoUrl),
    deleteUpload(project?.afterVideoUrl),
  ]);
...
export async function saveTestimonialAction(id: string | null, formData: FormData) {
  ...
  const payload = {
    name: str(formData, "name"), role: str(formData, "role"), company: str(formData, "company"), quote: str(formData, "quote"),
    avatarPalette: [str(formData, "paletteFrom") || "#ff6a00", str(formData, "paletteTo") || "#1a1a1d"] as [string, string], avatarUrl,
  };
  if (id) await updateTestimonial(id, payload); else await createTestimonial(payload);
  revalidateAll();
}
...
export async function saveClientAction(id: string | null, formData: FormData) {
  ...
  const avatarUrl = await resolveMedia(formData, "avatarFile", "avatarUrl", "removeAvatar", existing?.avatarUrl);
  const payload = {
    name: str(formData, "name"), platform: str(formData, "platform") as ClientPlatform, followers: str(formData, "followers"),
    avatarPalette: [str(formData, "paletteFrom") || "#ff6a00", str(formData, "paletteTo") || "#1a1a1d"] as [string, string], avatarUrl,
  };
...
export async function updateSettingsAction(formData: FormData) {
  ...
  const showreelVideoUrl = await resolveMedia(
    formData,
    "showreelVideoFile",
    "showreelVideoUrl",
    "removeShowreelVideo",
    current.showreelVideoUrl
  );
