    ...
    <section id="showreel" className="relative py-32">
      <Container>
        ...
        <motion.div
          ...
          className="group relative mt-14 aspect-video w-full overflow-hidden rounded-[2rem] border border-line"
        >
          <div
            ...
            style={{
              ...
              backgroundImage:
                "radial-gradient(90% 120% at 80% 20%, rgba(255,106,0,0.28), transparent 60%), linear-gradient(140deg, #0c0c0d 0%, #050505 70%)",
            }}
          />
          ...
          <button
            ...
            data-cursor="hover"
            aria-label="Reproducir showreel completo"
          ...
          <motion.div
            ...
            className="absolute inset-x-0 bottom-0 flex flex-col gap-4 bg-gradient-to-t from-black/85 via-black/40 to-transparent p-8 sm:flex-row sm:items-end sm:justify-between"
          >
            <div>
              <p className="text-xs uppercase tracking-[0.25em] text-mist">Duración</p>
              <p className="mt-1 font-display text-lg">{settings.showreelDuration}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-[0.25em] text-mist">Tipo de proyecto</p>
              <p className="mt-1 font-display text-lg">{settings.showreelType}</p>
            </div>
            <div>
              <p className="text-xs uppercase tracking-[0.25em] text-mist">Software</p>
              <p className="mt-1 font-display text-lg">{settings.showreelSoftware.join(" · ")}</p>
            </div>
          </motion.div>
        </motion.div>
      </Container>
